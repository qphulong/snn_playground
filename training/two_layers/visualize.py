...
Each of the four main plot types now has two versions:
...
if "we_pairs" in keys and "we_values" in keys and "we_times_ms" in keys:
    ...
    for k, (pi, pj) in enumerate(pairs):
        ...
        plt.tight_layout()
        save(fig, f"weight_evolution_all_in{pi:04d}_hid{pj:04d}.png")
    ...
    if "we_sample_values" in keys and "we_sample_times" in keys and "we_n_samples" in keys:
        ...
        for s in _sample_indices(n_total):
            ...
            for k, (pi, pj) in enumerate(pairs):
                ...
                ax.set_title(
                    f"Weight Evolution (sample {s}) — synapse in[{pi}] → hid[{pj}]",
                    fontsize=12, fontweight="bold"
                )
                ...
                ax.grid(True, alpha=0.3)
                plt.tight_layout()
                save(fig, f"weight_evolution_sample{s:03d}_in{pi:04d}_hid{pj:04d}.png")
...
def _plot_weight_matrix(W, title, filename):
...
if "final_weights_matrix" in keys:
    ...
    _plot_weight_matrix(W, "Final Weight Matrix (after all training)", "final_weight_matrix.png")
...
if "weight_matrix_per_sample" in keys and "weight_matrix_n_samples" in keys:
    ...
    for s in _sample_indices(n_total):
        ...
        _plot_weight_matrix(
            W,
            f"Weight Matrix after Sample {s}",
...
def _plot_firing_rate(rates, title, filename, color="darkorange"):
    n = len(rates)
...
if "mean_firing_rate_input" in keys:
    _plot_firing_rate(
        ...
        "Mean Firing Rate — Input Layer (all samples)",
...
if "mfr_in_sample_counts" in keys and "mfr_in_sample_dur_s" in keys and "mfr_in_n_samples" in keys:
    ...
    for s in _sample_indices(n_total):
        ...
        _plot_firing_rate(
            rates,
            f"Mean Firing Rate — Input Layer  |  Sample {s}",
...
if "mean_firing_rate_hidden" in keys:
    _plot_firing_rate(
        ...
        "Mean Firing Rate — Hidden Layer (all samples)",
...
if "mfr_hid_sample_counts" in keys and "mfr_hid_sample_dur_s" in keys and "mfr_hid_n_samples" in keys:
    ...
    for s in _sample_indices(n_total):
        ...
        _plot_firing_rate(
            rates,
            f"Mean Firing Rate — Hidden Layer  |  Sample {s}",
