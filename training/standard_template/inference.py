...
inference.py
============
Run the trained SNN on inference audio files and compute pairwise
similarity using the spikernel.

Loads weights from a history_epoch_*.npz produced by train.py + recorder.py.
The weight key is 'in_to_hid__final_weights' (refactor recorder format).

Usage:
    python inference.py
    # Edit WEIGHTS_PATH, INFER_DIR, NEURON_INDICES below as needed.
"""
...
WEIGHTS_PATH = os.path.join(SCRIPT_DIR, "history_epoch_015.npz")
INFER_DIR    = "datasets/vox1_single_person_nano_2/test"
OUTPUT_DIR   = os.path.join(SCRIPT_DIR, "vizs")
...
# ── Choose which neurons to include (empty = all) ─────────────────────────────
NEURON_INDICES = []

# ─────────────────────────────────────────────────────────────────────────────
# Network hyperparameters  (must match training/refactor/train.py)
# ─────────────────────────────────────────────────────────────────────────────
...
w_matrix = data["in_to_hid__final_weights"].astype(np.float64)
...
# ─────────────────────────────────────────────────────────────────────────────
# Network runner  (simplified: no STDP, no WTA — fixed weights only)
# ─────────────────────────────────────────────────────────────────────────────
...
def spikes_to_binned_sequence(spike_i, spike_t_ms, neuron_indices, duration_ms, bin_ms):
    """(len(neuron_indices), n_bins) spike-count array for the given neuron subset."""
...
print(f"\nNeuron indices used ({len(NEURON_INDICES)}): {NEURON_INDICES[:10]}{'...' if len(NEURON_INDICES) > 10 else ''}")
print("=" * 60)
...
n = len(sequences)
print(f"\nComputing {n}×{n} normalised spikernel matrix …")
...
make_heatmap(
    K_norm, labels,
    title=f"Spikernel similarity  (N_neurons={len(NEURON_INDICES)}, N_MAX={N_MAX}, λ={LAM}, μ={MU}, bin={BIN_MS}ms)",
