...
Shard and chunk the VoxCeleb2 `dev` pool for upload to Kaggle.

`datasets/vox2/dev` holds the full VoxCeleb2 training pool — ~6,112 speakers
in `speaker_id/session_id/utterance.m4a` layout (AAC, 16 kHz, mono). This
script reorganises it into speaker shards of 200 and consolidates the many
short utterances per session into uniform ~5-second clips, so the dataset is
small enough (per shard) and tidy enough to upload.
...
------------
  ...
  * The cut uses ffmpeg stream-copy (-c copy): no re-encode, so it is fast and
    bit-identical. Clips are ~5 s (a hair over, since cuts land on AAC packet
    boundaries) rather than exactly 5 s.
  * A trailing clip shorter than 2.5 s is dropped.

Output layout (inside datasets/vox2/):
...
its ORIGINAL directory under datasets/vox2/dev is DELETED to free space. The
job is resumable — interrupt it any time and re-run; state is re-derived purely
from what is on disk.

Dummy mode
----------
    python util_scripts/clean_vox2.py --dummy
...
Real run
--------
    python util_scripts/clean_vox2.py

Processes all remaining speakers, deleting originals as it goes.

Corrupted files are skipped and logged to corrupted_files_vox2.log.
"""
...
SRC_DEV        = Path("datasets/vox2/dev")     # source speaker pool
SHARD_ROOT     = Path("datasets/vox2")         # shards written here: dev_01, ...
...
CORRUPT_LOG    = Path("corrupted_files_vox2.log")
...
def process_session(src_session: Path, dst_session: Path) -> int:
    ...
    ~5-second clips written to `dst_session`. Returns the number of clips kept.
    """
    utterances = sorted(src_session.glob("*.m4a"))
    if not utterances:
        return 0
